...
def get_right_path (path):
	if path[0] == '/': 
		...
		while path.find('../') == 0:
			path = path[3:]
			prepath = prepath[:prepath.rfind("/")]
		truepath = prepath + "/"+ path
	return truepath
...
def trimming (library_file, path, commands, job_output, output, remove_originals):
	...
	for i in paired_list:
		if i[0][:1] == './': i[0] = i[0][1:]
		if i[1][:1] == './': i[1] = i[1][1:]
		output_string = output_string + "java -jar " + trimmo_exec + " PE -phred" + str(i[2]) + " " + get_right_path(i[0]) + " " + get_right_path(i[1]) + " "+output+"parsed_paired_"+ \
	...
	for i in single_list:
		if i[0] == '.': i = i[1:]
		output_string = output_string + "java -jar " + trimmo_exec + " SE -phred" + str(i[2]) + " " + get_right_path(i[0]) + " " + get_right_path(i[1]) + " " +output+i[0][:i[0].rfind("/")+1]+"parsed_"+\
